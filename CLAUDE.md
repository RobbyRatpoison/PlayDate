...
- **`backfill_metadata(appid, *, force=False)`**: returns a dict for `update_game_data` with **only fields currently empty on the game** (plugin data always wins). Tiers: Steam AppID → `fetch_store_data`/`fetch_review_data`/`fetch_tag_data` (tags + reviews), PCGW box backfills anything Steam missed. No Steam AppID but a PCGW page (store-exclusive classics) → dev/pub/genres/categories/release from the infobox, no tags/reviews. **Still no Steam page → the platform's own store page** via `_store_page_metadata()` → `plugin.store_page_metadata(platform_slug)` (only itch.io implements it — its API has no taxonomy) fills genres/tags/release; itch tags are filtered to the library's existing tag vocabulary so the set stays consistent. Never touches name, achievements, playtime, art. Outcome → `games.meta_backfill_fetched` (`0`/NULL = never, YYYY-MM-DD, `no_match`, `unconfirmed`); resolved id → `games.steam_appid`. `force=True` applies a `maybe` match and re-runs a done game.
