...
- **`_pcgw_infobox(name, session) -> dict | None | _PCGW_UNAVAILABLE`**: `list=search` for the page → `action=parse&section=0` → parse the `{{Infobox game}}` wikitext (follows one `#REDIRECT` hop). Returns whatever it has: `steam_appid`, `gog_id`, `developers`/`publishers` (from `{{Infobox game/row/developer|X}}` rows), `genres` and `categories` (from `taxonomy/genres` and `taxonomy/modes` → Single-player/Multi-player), and the earliest `release_date` across the `taxonomy`… `row/date` rows. **PCGW throttles hard** — `_pcgw_get()` serialises requests (1.5s min gap) and stands down 180s on a 429; `_PCGW_UNAVAILABLE` is the "couldn't answer" sentinel (distinct from "answered, nothing there").
- **`_resolve(name) -> (steam_appid|None, confidence, pcgw_box|None)`** / **`resolve_steam_appid(name)`** (2-tuple wrapper): PCGW box first; if it has a Steam AppID → `'pcgw'`. Else Steam `storesearch` gated by `difflib`: `>= _SIM_CONFIRMED` (0.90) → `'confirmed'`; `_SIM_MAYBE`..`_SIM_CONFIRMED` → `'maybe'` (parked as `unconfirmed`, nothing written); PCGW throttled + only a weak guess → `'retry'` (record nothing, next run).
- **`backfill_metadata(appid, *, force=False)`**: returns a dict for `update_game_data` with **only fields currently empty on the game** (plugin data always wins). With a Steam AppID → `fetch_store_data`/`fetch_review_data`/`fetch_tag_data` (tags + reviews), PCGW box backfills anything Steam missed. With a PCGW page but **no Steam AppID** (store-exclusive classics) → dev/pub/genres/categories/release from the infobox, no tags/reviews. Never touches name, achievements, playtime, art. Outcome → `games.meta_backfill_fetched` (`0`/NULL = never, YYYY-MM-DD, `no_match`, `unconfirmed`); resolved id → `games.steam_appid`. `force=True` applies a `maybe` match and re-runs a done game.
